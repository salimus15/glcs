...
PetscErrorCode real2complexMat(Mat *A,Mat * r2c){
	...
	// on garde des matrices avec les parties réelles
	// et imaginaires	
	ierr=MatRealPart(R);CHKERRQ(ierr);
	ierr=MatImaginaryPart(C);CHKERRQ(ierr);	
	
	// on récupère les valeurs des deux matrices dans des tableaux
...
/*
PetscErrorCode real2complex_MatMult(Mat A, Vec x, Vec y){
	void * ctx;
	PetscInt sizen, sizem;
	
	MatGetLocalSize(A,sizem,sizen)
	
	MatCreateShell(PETSC_COMM_WORLD,sizem,sizen,PETSC_DECIDE,PETSC_DECIDE,ctx,&A);
